...
RSpec.describe 'Players API', type: :request do
  ...
  let(:player_pid) {players.first.player_id}
  ...
  describe 'GET /players?name=' do
    ...
    let(:player_pid_2) {players.last.player_id}
    ...
    context 'when looking for a specific player by name' do
      ...
      it 'returns the player rushing, kicking, passing, receiving stats' do
        ...
        expect(json['rushing'].first['entry_id']).to eq(rushing['entry_id'])
        ...
        expect(json['receiving'].first['entry_id']).to eq(receiving['entry_id'])
        ...
        expect(json['kicking'].first['entry_id']).to eq(kicking['entry_id'])
        ...
        expect(json['passing'].first['entry_id']).to eq(passing['entry_id'])
      end
    ...
    context 'when looking for more than one player by name' do
      ...
      it 'should return two correct user names' do
        ...
        expect(json['rushing'].last['entry_id']).to eq(rushing_2['entry_id'])
  ...
  describe 'POST /players' do
    # valid payload
    let(:valid_attributes) {{name: 'Kenneth Chen', position: 'QA', player_id: '1234'}}
    ...
    context 'when player with same name but different pid is entered' do
      before {post '/players', params: valid_attributes}
      before {post '/players', params: {name: 'Kenneth Chen', position: 'QA', player_id: '12345'}}
      before {get '/players'}
    ...
    context 'when player with same pid is entered again' do
      before {post '/players', params: valid_attributes}
      before {post '/players', params: {name: 'Jack Ma', position: 'QA', player_id: '1234'}}
      before {get '/players'}
